...
- (id)init
{
    if ((self =[super init])) 
    {
        ...
        for (int i = 0; i < [allImageNames count]; i ++) 
        {
...
- (void)viewDidLoad
{
    ...
    for (int i=1; i<=34; i++)
    {
        [allImageNames addObject:[NSString stringWithFormat:@"image%d.png", i]];
    }
    NSLog(@"image names %@", allImageNames);
...
- (GMGridViewCell *)GMGridView:(GMGridView *)gridView cellForItemAtIndex:(NSInteger)index
{
  ...
  //  label.text = (NSString *)[allImageNames objectAtIndex:index];
    label.textAlignment = UITextAlignmentCenter;
   // label.backgroundColor = [UIColor clearColor];
    label.textColor = [UIColor blackColor];
    label.highlightedTextColor = [UIColor whiteColor];
    label.font = [UIFont boldSystemFontOfSize:20];
    label.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:[NSString stringWithFormat:[allImageNames objectAtIndex:index]]]];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 1) 
    {
        [allImageNames removeObjectAtIndex:_lastDeleteItemIndexAsked];
...
{
    NSObject *object = [allImageNames objectAtIndex:oldIndex];
    [allImageNames removeObject:object];
    [allImageNames insertObject:object atIndex:newIndex];
}

- (void)GMGridView:(GMGridView *)gridView exchangeItemAtIndex:(NSInteger)index1 withItemAtIndex:(NSInteger)index2
{
    [allImageNames exchangeObjectAtIndex:index1 withObjectAtIndex:index2];
}
...
- (void)refreshItem
{
    ...
    if ([allImageNames count] > 0) 
    {
        int index = [allImageNames count] - 1;
        
        NSString *newMessage = [NSString stringWithFormat:@"%d", (arc4random() % 1000)];
        
        [allImageNames replaceObjectAtIndex:index withObject:newMessage];
