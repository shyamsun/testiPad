...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	if ([self touchedPrevPage] && [self hasPrevPage]) {
        ...
        NSLog(@"began3");
        vc1=[[ProductDescriptionViewController alloc] initWithNibName:@"ProductDescriptionViewController" bundle:nil];
        [self.viewController presentPopupViewController:vc1 animationType:MJPopupViewAnimationSlideBottomBottom];        
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	[CATransaction begin];
	float duration;
	if ((dragged && self.leafEdge < 0.95) || (!dragged && [self touchedNextPage])) {
