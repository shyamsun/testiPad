...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    for (NSUInteger i=0; i<[DMPGridViewCell cellsPerRowForStyle:style]; i++) {
        ...
        dispatch_async(imageDownloadQ, ^{
            couponsAdsObject *coupon = [self.list objectAtIndex:(offset + i)];
            NSURL *imageURL = coupon.couponPhotoUrl;
        ...
        // Title
        couponsAdsObject *coupon = [self.list objectAtIndex:(offset + i)];
        NSString *text = [NSString stringWithFormat:@"%@", coupon.title]; // ... todo fix
...
- (void)tap:(UIGestureRecognizer *)gestureRecognizer {
    couponsAdsObject *coupon = [self.list objectAtIndex:gestureRecognizer.view.tag];
    ProductDescriptionViewController *product = [[ProductDescriptionViewController alloc] initWithNibName:@"ProductDescriptionViewController" withImage:coupon.couponPhotoUrl withTitle:coupon.title withPrice:nil bundle:nil];
